            ...
            <div class="row">
                ...
                <div class="form-group col-sm-3">
                    ...
                    <select name='sec_data' class="form-control select2" required>
                        <?php
                        $secq = "SELECT * FROM sec 
                         where  course_id=1 and sem in (6,8) ANd possision='active' ORDER BY course_id, dept_id, sem, section";
				...
				<div class="form-group col-sm-3">
					...
					<!--  date('j F, Y') -->
                   <input type="text" name="wef_dt" value="01 December, 2025">
                </div>
...
<table border='1' id='data_tbl' style='font-family:'Times New Roman'>
        ...
        <tr>
            ...
            <td style='border: none; padding: 5px;'>DEAN-SLAM</td>
    ...
    function allocatePeriods(&$timetable, $subjects, $index, $dbconn, $ay, $sem_type,$sec_id) {
        ...
        for ($d = 0; $d < $no_of_days; $d++) {
            for ($p = 0; $p < $no_of_periods; $p++) {
				...
				/*if (isAvailable($timetable, $d, $p) && isStaffAvailable($dbconn, $staff, $d, $p, $ay, $sem_type) &&
     (!$isTheory || isOneTheoryPerDay($timetable, $code, $d))
	) {
                    $slots[] = [$d, $p];
                }*/
				...
				/*if (
					...
					(
						!$isTheory || ( isOneTheoryPerDay($timetable, $code, $d) &&
							isOneTheoryPerDayPerStaffSec($timetable, $code, $staff, $d, $sec_id)
						)
					)
				) 
				{
    ...
    WHERE 
	  ...
	  $second_fac_message="";
		...
		$fetch_query = "SELECT ay, sec_id, subject_code, day_no, st_period, end_period, staff_idno, room_no, sem_type
                ...
                WHERE ay = ? AND sec_id = ? 
				AND faculty_type in( 'SECOND','THIRD','FOURTH')";

...
if (isset($_POST['sec_ok'])) 
{
   ...
   // $regulation = $_POST['regulation'];
	...
	function showTimetable($ay, $sec_id, $sec_name,$disp_br_name,$disp_degree_sem,$section,$wef_dt,$dept_id,$sem)
	{
		...
		FROM (
		SELECT 
        GROUP_CONCAT(DISTINCT t1.staff_idno ORDER BY t1.staff_idno) AS staff_ids,
        t1.ay, 
        t1.sec_id, 
        t1.day_no, 
        t1.st_hour,
		...
		if ($res->num_rows > 0) {
			...
			while ($row = $res->fetch_assoc()) {
				...
				$timetable[$day][$period] = [
					...
					'staff' => "EID: ".substr($row['staff_ids'],4,5),
					'room_no' => $row['room_no'],
					'type' => 'T'
				];
			}
...
<table border='1' id='data_tbl' style='font-family:'Times New Roman'>
			...
			if (mysqli_num_rows($result) > 0) {
				...
				while ($row = mysqli_fetch_assoc($result)) {
					...
					<td>{$row['subject_code']}</td>
							...
							<td>EID: ".substr($row['staff_idno'],4,5)."</td>
    ...
    $stmt = $dbconn->prepare("SELECT subject_code, day_no, st_period, end_period, staff_idno FROM predefined_timetable WHERE ay = ? AND sec_id = ? and faculty_type='MAIN'"); // CONSIDER MAIN
    $stmt->bind_param("si", $ay, $sec_id);
    $stmt->execute();
    $res = $stmt->get_result();
    while ($row = $res->fetch_assoc()) {
    ...
    $stmt = $dbconn->prepare("SELECT subject_code, staff_idno, teaching_periods 
        FROM subject_staff_mapping 
        WHERE ay = ? AND sec_id = ? 
    ...
    function isStaffAvailable($dbconn, $staff, $day, $period, $ay, $sem_type) {
        ...
        $stmt->close();
        // check predefined also
        //SELECT `id`, `ay`, `sec_id`, `subject_code`, `day_no`, `st_period`, `end_period`,
        //  `staff_idno`, `room_no`, `slot_status` FROM `predefined_timetable` WHERE 1
    ...
    // Step 5: Allocate all theory periods
    ...
    echo "<table border='1'>
	<tr><td colspan=7>".$sec_name."</td></tr>
	<tr><th>Day / Period</th>";

...
<head>
    ...
    $('#dateRangeForm').submit(function(event) {
        ...
        $.ajax({
            url: 'generate_consolidate_proqurement.php',
            type: 'POST',
...
<body>
	<?= include('./menu.php') ?>
    <h1>Consolidate Proqurement Cost Report</h1>
    <form id="dateRangeForm">
        <label for="startdate">Start Date:</label>
        <input type="date" id="startdate" name="startdate" class='form-control' value='<?=$_GET['startdate']?>' required>
        
        <label for="enddate">End Date:</label>
        <input type="date" id="enddate" name="enddate" class='form-control' value='<?=$_GET['enddate']?>' required>

    ...
    {   // Generate new timetable
     ...
     // 3. Allocate theory periods with safety for infinite loops
        ...
        foreach ($theoryCourses as $course) 
        {
            ...
            {				
                echo "<div class='alert alert-warning'>$subject</div>";
				$could_not_allocate[$ci]=$subject;
				++$ci;
				//echo"<br> NA";
        ...
        for ($d = 0; $d < 6; $d++) 
        {
            ...
            for ($p = 0; $p < 6; $p++) 
            {
                ...
                else 
                {
					...
					echo "<td class='not-allocated'> ".$missing_subject."</td>";
...
if (isset($_POST['lock_btn'])) {
    ...
    // ✅ Validate for any unallocated periods
    $hasEmpty = false;
    foreach ($timetable as $day => $periods) {
        foreach ($periods as $p => $entry) {
            if (!is_array($entry)) {
                echo "<div class='alert alert-danger'>Period not allocated at Day " . ($day + 1) . ", Period " . ($p + 1) . "</div>";
                $hasEmpty = true;
            }
        }
    }
    ...
    if ($hasEmpty) {
        echo "<div class='alert alert-danger'>Cannot lock timetable. Some periods are not allocated. Please fix and try again.</div>";
        exit;
    }

    // ✅ DELETE existing entries
    $delQuery = "DELETE FROM timetable WHERE ay = ? AND sec_id = ? AND sem_type = ?";
    ...
    // ✅ INSERT new entries
    $insertQuery = "INSERT INTO timetable (staff_idno, day_no, sec_id, sem, st_hour, end_hour, subject_code, room_no, duration, ay, locked, sem_type) 
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    ...
    foreach ($timetable as $day => $periods) {
        ...
        while ($p < 6) {
            ...
            if ($type === 'L') {
                while ($end < 6 && isset($periods[$end]) && is_array($periods[$end]) &&
                       $periods[$end]['code'] === $subject && $periods[$end]['type'] === 'L') {
                    $end++;
                }
                $end--;
            }
            ...
            $stmt->bind_param("siiiiissisis",
                $staff, $day_no, $sec_id, $sem, $st_hour, $end_hour,
                $subject, $room_no, $duration, $ay, $locked, $sem_type
            );

            if (!$stmt->execute()) {
                echo "<div class='alert alert-danger'>Insert failed at Day $day_no Period $st_hour: " . $stmt->error . "</div>";
            }

...
<script>
    $(document).ready(function () {
        $('.select2').select2({
            tags: true, // ✅ Enables custom entry
            theme: 'bootstrap4',
    ...
    if (!isset($_SESSION['timetable'])) {
		...
		foreach ($timetable as $day => $periods) 
		{
			...
			while ($p < 6) 
			{
				...
				// Merge consecutive periods if LAB
				if ($type === 'L') 
				{
       ...
       /* foreach ($timetable as $day => $periods) 
        {
            ...
            while ($p < 6) 
            {
                ...
                $ct_no = null; // or generate if needed
                $room_no = "";
                $duration = 1; // you can also set it as $end - $start + 1
                $locked = 1;
				$day_no = $day + 1;
                $stmt->bind_param("siiiiiisssis", 
                    $staff, $day_no, $sec_id, $sem, $start, $end, 
                    $subject, $room_no, $duration, $ay, $locked, $sem_type
                );
                $stmt->execute();
        ...
        // Clear session to prevent re-saving

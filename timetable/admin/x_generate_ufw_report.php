...
$sql = "SELECT distinct issue_category, food_name, food_weight
        FROM wastage_food 
        WHERE wastage_date = '$start_date' 
        ORDER BY issue_category, food_name";

$result = mysqli_query($dbconn, $sql);

// Define category names and initialize tracking
$issue_cat_arr = ['B' => 'Breakfast', 'L' => 'Lunch', 'S' => 'Snacks', 'D' => 'Dinner'];
$food_issue_arr = ['B' => [], 'L' => [], 'S' => [], 'D' => []]; // Store data per category

// Store fetched data in categorized array
while ($row = mysqli_fetch_assoc($result)) {
	if ($row['food_weight'] > 0.0) 
	{
		$category = $row['issue_category'];
		$food_issue_arr[$category][] = $row;
	}
}
//SELECT *,count(*) FROM `wastage_food` group by wastage_date,issue_category,item_id ORDER BY `count(*)` DESC
// Start table output
echo "<table border='1' style='border-collapse:collapse;width:80%;' align='center' cellpadding='10'>
    <tr><th colspan='4'>Unutilized Cooked Food Report</th></tr>
    <tr><td colspan='2' style='text-align:left'>Total Students Dined: <b>".$tot_din."</b> </td>
    <td colspan='2' style='text-align:right'>$print_dates</td></tr>
    <tr><th>S.No</th><th>Food Category</th><th>Menu Item</th><th>Unutilized Cooked <br>Food Weight<br>(Kg./Ltr.)</th></tr>";

$sno = 0;

// Loop through categories in the fixed order
foreach ($issue_cat_arr as $key => $category_name) {
	//added now   
	$food_items = $food_issue_arr[$key] ?? [];
    $valid_items = array_filter($food_items, function($item) {
        return $item['food_weight'] > 0.00; // Exclude items with 0 weight
    });
	//ends here
	
    //if (empty($food_issue_arr[$key])) {
	if(empty($food_items) || empty($valid_items)){
		
        // If category is missing, print NIL row
        echo "<tr><td style='text-align:center'>" . (++$sno) . "</td>
              <td style='text-align:center;font-weight:bold'>" . $category_name . "</td>
              <td colspan='2' style='text-align:center;font-weight:bold'>NIL</td></tr>";
    } else {
        // Print rows with rowspan for category
        $first = true;
        ...
        foreach ($food_issue_arr[$key] as $item) {
            ...
            echo "<td style='text-align:center'>" . $item['food_name'] . "</td>
                  <td style='text-align:center'>" . number_format($item['food_weight'], 2) . "</td></tr>";
        }
    }
}
